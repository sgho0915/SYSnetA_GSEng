namespace UIWidgets
{
	...
	public class ScrollRectPaginator : MonoBehaviour, IStylable, IUpgradeable
	{
		...
		/// </summary>
		protected virtual bool IsValid => ScrollRect != null && ScrollRect.content != null;
...
#pragma warning restore 0649
		...
		protected RectTransform PagesContainer
		{
			get
			{
				...
				{
					return SRDefaultPage.transform.parent as RectTransform;
				}
		...
		public virtual void Init()
		{
			...
			if (DefaultPage != null)
			{
				...
				{
					defaultPagesCache = new BasePool<ScrollRectPage>(SRDefaultPage);
				}
			}
		...
		protected virtual void RemoveListeners(ScrollRect scroll)
		{
			...
			var resize = scroll.GetComponent<ResizeListener>();
			if (resize != null)
			{
				resize.OnResizeNextFrame.RemoveListener(RecalculatePages);
			}

			var drag = scroll.GetComponent<DragListener>();
			if (drag != null)
			{
			...
			var scroll_listener = scroll.GetComponent<ScrollListener>();
			...
			{
				var content_resize = scroll.content.GetComponent<ResizeListener>();
				if (content_resize != null)
				{
		...
		protected virtual void UpdateObjects(int page)
		{
			...
			if (defaultPagesCache.Template != null)
			{
				...
				SRActivePage.gameObject.SetActive(IsValid);
				...
				LayoutUtilities.UpdateLayout(PagesContainer.GetComponent<LayoutGroup>());
			}
